...
interface DemographicData {
    ...
    voters: { registered: number; unregistered: number; maleVoters: number; femaleVoters: number };
    religion: { name: string; count: number }[];
    civilStatus: { status: string; male: number; female: number }[];
    beneficiaries: { nhts4ps: number; nhtsNon4ps: number; nonNhts: number };
    others: { deceased: number; indigenous: number };
}
...
interface Props {
    originalData: DemographicData;
    filteredData: DemographicData | null;
    filters?: FilterProps;
}
...
export default function DemographicStats({ originalData, filteredData, filters }: Props) {
    // Date State
    ...
    const data = filteredData || originalData;

    // Helper for cross-out values
    const ValDisp = ({ orig, curr }: { orig: number | string, curr: number | string }) => {
        if (!hasFilter) return <span>{orig}</span>;
        return (
            <span className="inline-flex items-center gap-1.5 flex-wrap">
                <span className="line-through text-neutral-400 dark:text-neutral-500 font-normal opacity-70 text-[0.8em]">
                    {orig}
                </span>
                <span>{curr}</span>
            </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                        ...
                        <div className="grid grid-cols-2 gap-8 z-10">
                            ...
                            <div className="flex flex-col justify-center">
                                ...
                                <div className="text-5xl font-black text-neutral-900 dark:text-neutral-100 mb-4 tracking-tighter">
                                    <ValDisp orig={originalData.totalPopulation.toLocaleString()} curr={data.totalPopulation.toLocaleString()} />
                                </div>
                                <div className="flex gap-4">
                                    <div className="flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-blue-500" />
                                        <span className="text-xs text-neutral-500 font-medium whitespace-nowrap">
                                            Male: <ValDisp orig={originalData.gender.male.toLocaleString()} curr={data.gender.male.toLocaleString()} />
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-pink-500" />
                                        <span className="text-xs text-neutral-500 font-medium whitespace-nowrap">
                                            Female: <ValDisp orig={originalData.gender.female.toLocaleString()} curr={data.gender.female.toLocaleString()} />
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                {data.ageGroups.map((group, idx) => {
                                    const origGroup = originalData.ageGroups[idx];
                                    return (
                                        <div key={idx} className="flex flex-col gap-1">
                                            <div className="flex justify-between text-[10px] font-bold text-neutral-500 uppercase">
                                                <span>{group.label}</span>
                                                <span><ValDisp orig={origGroup.count} curr={group.count} /></span>
                                            </div>
                                            <div className="w-full h-1.5 bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden">
                                                <div
                                                    className={`h-full ${group.color} rounded-full`}
                                                    style={{ width: `${(group.count / Math.max(1, data.totalPopulation)) * 100}%` }}
                                                />
                                            </div>
                                        </div>
                                    );
                    ...
                    <div className="bg-white dark:bg-sidebar border border-sidebar-border rounded-2xl p-6 shadow-sm flex flex-col">
                        ...
                        <div className="flex-1 flex flex-col gap-4">
                            <StatRow label="Registered Voters" origValue={originalData.voters.registered} value={data.voters.registered} total={data.totalPopulation} color="bg-purple-500" hasFilter={hasFilter} />
                            <StatRow label="Unregistered (18+)" origValue={originalData.voters.unregistered} value={data.voters.unregistered} total={data.totalPopulation} color="bg-neutral-300" hasFilter={hasFilter} />
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-2 pt-4 border-t border-dashed border-sidebar-border">
                                <div className="text-center p-3 bg-neutral-50 dark:bg-neutral-900/20 rounded-lg">
                                    ...
                                    <span className="text-lg font-bold text-neutral-800 dark:text-neutral-200">
                                        <ValDisp orig={originalData.voters.maleVoters} curr={data.voters.maleVoters} />
                                    </span>
                                </div>
                                <div className="text-center p-3 bg-neutral-50 dark:bg-neutral-900/20 rounded-lg">
                                    <span className="block text-[10px] uppercase font-bold text-neutral-400">Female</span>
                                    <span className="text-lg font-bold text-neutral-800 dark:text-neutral-200">
                                        <ValDisp orig={originalData.voters.femaleVoters} curr={data.voters.femaleVoters} />
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-white dark:bg-sidebar border border-sidebar-border rounded-2xl p-6 shadow-sm flex flex-col">
                        ...
                        <div className="space-y-4">
                            <div className="p-4 rounded-xl bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/20 flex justify-between items-center">
                                ...
                                <span className="text-lg font-black text-orange-800 dark:text-orange-200">
                                    <ValDisp orig={originalData.beneficiaries.nhts4ps} curr={data.beneficiaries.nhts4ps} />
                                </span>
                            </div>
                            <div className="flex justify-between items-center px-2">
                                <span className="text-xs font-medium text-neutral-500">NHTS Non-4Ps</span>
                                <span className="text-sm font-bold text-neutral-800 dark:text-neutral-200">
                                    <ValDisp orig={originalData.beneficiaries.nhtsNon4ps} curr={data.beneficiaries.nhtsNon4ps} />
                                </span>
                            </div>
                            <div className="flex justify-between items-center px-2">
                                <span className="text-xs font-medium text-neutral-500">Non-NHTS</span>
                                <span className="text-sm font-bold text-neutral-800 dark:text-neutral-200">
                                    <ValDisp orig={originalData.beneficiaries.nonNhts} curr={data.beneficiaries.nonNhts} />
                                </span>
                            </div>
                            ...
                            <div className="pt-4 border-t border-sidebar-border mt-4 space-y-2">
                                <div className="flex items-center gap-2 text-xs text-neutral-500">
                                    ...
                                    <span>Deceased: <b className="ml-1"><ValDisp orig={originalData.others.deceased} curr={data.others.deceased} /></b></span>
                                </div>
                                <div className="flex items-center gap-2 text-xs text-neutral-500">
                                    <Flag className="size-3 text-neutral-400" />
                                    <span>Indigenous People: <b className="ml-1"><ValDisp orig={originalData.others.indigenous} curr={data.others.indigenous} /></b></span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-2 bg-white dark:bg-sidebar border border-sidebar-border rounded-2xl p-6 shadow-sm overflow-hidden">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                {data.civilStatus.map((item, idx) => {
                                    const origItem = originalData.civilStatus.find(i => i.status === item.status) || { male: 0, female: 0 };
                                    return (
                                        <tr key={idx} className="hover:bg-neutral-50 dark:hover:bg-neutral-900/20">
                                            ...
                                            <td className="px-4 py-3 text-center font-mono text-neutral-500"><ValDisp orig={origItem.male} curr={item.male} /></td>
                                            <td className="px-4 py-3 text-center font-mono text-neutral-500"><ValDisp orig={origItem.female} curr={item.female} /></td>
                                            <td className="px-4 py-3 text-right font-bold text-neutral-900 dark:text-neutral-100">
                                                <ValDisp orig={origItem.male + origItem.female} curr={item.male + item.female} />
                                            </td>
                                        </tr>
                                    );
                                })}
                    ...
                    <div className="md:col-span-2 bg-white dark:bg-sidebar border border-sidebar-border rounded-2xl p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {data.religion.map((rel, idx) => {
                                const origRel = originalData.religion.find(r => r.name === rel.name) || { count: 0 };
                                return (
                                    <div key={idx} className="flex justify-between items-center p-3 rounded-lg border border-sidebar-border/60 hover:bg-neutral-50 dark:hover:bg-neutral-900/20 transition-colors">
                                        ...
                                        <span className="text-sm font-bold font-mono text-neutral-900 dark:text-neutral-100">
                                            <ValDisp orig={origRel.count} curr={rel.count} />
                                        </span>
                                    </div>
                                );
                            })}
...
function StatRow({ label, origValue, value, total, color, hasFilter }: { label: string, origValue: number, value: number, total: number, color: string, hasFilter: boolean }) {
                    ...
                    <span className="text-sm font-bold text-neutral-900 dark:text-neutral-100 mr-1">
                        {hasFilter ? (
                            <span className="inline-flex items-center gap-1.5">
                                <span className="line-through text-neutral-400 font-normal opacity-70 text-[0.8em]">
