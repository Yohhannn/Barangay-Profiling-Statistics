import {
    ...
    Search, UserCheck, UserX, Construction, Loader2, Pencil
} from 'lucide-react';
...
interface InfrastructureEditProps {
    isOpen: boolean;
    onClose: () => void;
    sitios: Sitio[];
...
export default function InfrastructureEdit({ isOpen, onClose, sitios, infrastructure }: InfrastructureEditProps) {
    // Infra fields
    ...
    useEffect(() => {
        if (isOpen && infrastructure) {
            setInfraName(infrastructure.name);
            setAddress(infrastructure.address || '');
            setSitioId(String(infrastructure.sitioId));
            setType(infrastructure.type);
            setDescription(infrastructure.description || '');

            setOwnerFirst(infrastructure.ownerFname);
            setOwnerMiddle(infrastructure.ownerMname || '');
            setOwnerLast(infrastructure.ownerLname);
            setOwnerSuffix(infrastructure.ownerSuffix || '');
            setCtzId(infrastructure.ctzId);
            setIsLocked(!!infrastructure.ctzId);
            
            // If there's a citizen linked, we might want to display their UUID or name
            if (infrastructure.ctzId) {
                setOwnerUuid(`Linked Citizen ID: ${infrastructure.ctzId}`);
            } else {
                setOwnerUuid('');
            }
        }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.put(`/institutions/infrastructure/${infrastructure.id}`, {
            name: infraName,
            type,
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-[#F8F9FC] dark:bg-[#0f172a] w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] border border-white/20">
                ...
                {/* Header */}
                <div className="relative bg-sky-700 text-white p-5 flex justify-between items-center overflow-hidden shrink-0">
                    <div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none">
                        <Building className="w-32 h-32" />
                    </div>
                    <div className="z-10">
                        <h2 className="text-lg font-bold tracking-wide flex items-center gap-2">
                            <Pencil className="size-5 text-sky-200" />
                            Edit Infrastructure
                        </h2>
                        <p className="text-[10px] text-sky-200 uppercase tracking-widest font-semibold mt-1">
                            {infrastructure.infraId} — Modifying Infrastructure Record
                        </p>
                    </div>
                                ...
                                <div className="flex items-center justify-between p-3 bg-sky-50 dark:bg-sky-900/20 border border-sky-100 dark:border-sky-900/50 rounded-lg animate-in fade-in zoom-in">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs font-medium text-sky-900 dark:text-sky-100 font-mono">{ownerUuid || 'Linked Record'}</p>
                                        </div>
                                    </div>
                ...
                {/* Footer */}
                <div className="p-5 bg-white dark:bg-[#0f172a] border-t border-neutral-200 dark:border-neutral-800 flex justify-between items-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] shrink-0">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-8 py-2.5 bg-sky-600 hover:bg-sky-700 disabled:opacity-60 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-lg shadow-sky-600/20 active:scale-95"
                    >
