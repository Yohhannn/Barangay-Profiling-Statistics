...
interface Citizen {
    ...
    suffix?: string;
    sex: string;
    dob: string; // The backend should be formatted YYYY-MM-DD for standard html date inputs
    pob?: string;
    civilStatus?: string;
    religion?: string;
    bloodType?: string;
    ...
    membershipType?: string; // philhealth_membership
    isStudent: boolean;
    schoolName?: string;
    educAttainment?: string; // current_level
    // these are not mapped by backend currently for edit, but maybe in future:
...
// Mock Data for Household Lookup (Same as creation)
...
export default function CitizenEdit({ isOpen, onClose, citizen, onSuccess }: CitizenEditProps) {
    ...
    // Format Date exactly as needed for date inputs: "YYYY-MM-DD"
    // Note: If the backend sends 'F d, Y' (e.g. 'January 01, 1990'), we must parse it back.
    // Ensure that backend also returns standard 'YYYY-MM-DD' if possible for easy editing.
    // For now we assume we might need to handle standard format or use as is if backend sends standard.
    ...
    useEffect(() => {
        if (citizen) {
            setData({
                ...
                date_of_birth: formatDateForInput(citizen.dob),
                place_of_birth: citizen.pob || '',
                civil_status: citizen.civilStatus || '',
                religion: citizen.religion || '',
                blood_type: citizen.bloodType || '',
                ...
                relationship_to_head: citizen.relationship && citizen.relationship !== 'N/A' ? citizen.relationship : '',
                socio_economic_class: citizen.socioEconomicStatus && citizen.socioEconomicStatus !== 'N/A' ? citizen.socioEconomicStatus : '',
                nhts_number: citizen.nhtsNumber && citizen.nhtsNumber !== 'N/A' ? citizen.nhtsNumber : '',

                employment_status: citizen.employmentStatus && citizen.employmentStatus !== 'N/A' ? citizen.employmentStatus : '',
                occupation: citizen.occupation && citizen.occupation !== 'N/A' ? citizen.occupation : '',
                is_gov: citizen.isGovWorker || false,

                philhealth_id: citizen.philhealthId && citizen.philhealthId !== 'N/A' ? citizen.philhealthId : '',
                philhealth_category: citizen.philhealthCategory && citizen.philhealthCategory !== 'N/A' ? citizen.philhealthCategory : '',
                philhealth_membership: citizen.membershipType && citizen.membershipType !== 'N/A' ? citizen.membershipType : '',
                ...
                is_ip: citizen.isIp || false,
                is_deceased: citizen.isDeceased || false,
                date_of_death: formatDateForInput(citizen.dateOfDeath),
                cause_of_death: citizen.causeOfDeath && citizen.causeOfDeath !== 'N/A' ? citizen.causeOfDeath : '',

                fp_method: citizen.fpMethod && citizen.fpMethod !== 'N/A' ? citizen.fpMethod : '',
                ...
                <div className="bg-[#F8F9FC] dark:bg-[#0f172a] w-full max-w-5xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh] border border-white/20">
                    ...
                    <div className="relative bg-[#1e1b4b] text-white p-5 flex justify-between items-center overflow-hidden shrink-0">
                        ...
                        <div className="z-10">
                            ...
                            <p className="text-[10px] text-indigo-300 uppercase tracking-widest font-semibold mt-1">
                                Updating Record: {citizen.citizenId}
                            </p>
                        </div>
                    ...
                    <div className="p-5 bg-white dark:bg-[#0f172a] border-t border-neutral-200 dark:border-neutral-700 flex justify-end items-center">
                        <button
                            ...
                            disabled={processing}
                            className="flex items-center gap-2 px-8 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-lg active:scale-95 disabled:opacity-50"
                        >
