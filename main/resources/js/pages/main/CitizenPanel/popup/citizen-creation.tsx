import {
    ...
    CreditCard, Heart, Plus, Trash2, ScanFace
} from 'lucide-react';
                            ...
                            <div className="bg-white dark:bg-[#1e293b] p-6 rounded-xl border border-neutral-200 dark:border-neutral-700 shadow-sm flex flex-col sm:flex-row gap-6">
                                ...
                                <div className="flex flex-col gap-3 w-full sm:w-40 shrink-0">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <button className="flex items-center justify-center bg-[#1e1b4b] hover:bg-indigo-800 text-white p-2 rounded-lg transition-colors" title="Take Photo">
                                            <Camera className="size-4" />
                                        </button>
                                        <button className="flex items-center justify-center bg-white border border-neutral-300 hover:bg-neutral-50 dark:bg-neutral-800 dark:border-neutral-600 dark:text-white p-2 rounded-lg transition-colors" title="Upload">
                                            <Upload className="size-4" />
                                        </button>
                                    </div>

                                    {/* Register Face Recognition Button */}
                                    <button className="w-full flex items-center justify-center gap-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 p-2.5 rounded-lg transition-all text-[10px] font-bold uppercase tracking-wide shadow-sm mt-1 group">
                                        ...
                                        <div key={idx} className="flex gap-2">
                                            <div className="relative group flex-1">
                                                <input
                                                    ...
                                                    className="w-full text-xs p-2.5 rounded-lg border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100 placeholder:text-neutral-400 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all"
                                                />
                                                <div className="absolute right-3 top-2.5 text-neutral-400 group-focus-within:text-indigo-500 transition-colors">
                                                    <PhoneIcon />
                                                </div>
                                            </div>
                                            {idx === 0 ? (
                                                <button onClick={addContactNumber} className="p-2 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-colors border border-indigo-200">
                                                    <Plus className="size-4" />
                                                </button>
                                            ) : (
                                                <button onClick={() => removeContactNumber(idx)} className="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors border border-red-200">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white dark:bg-[#1e293b] p-6 rounded-xl border border-neutral-200 dark:border-neutral-700 shadow-sm space-y-5">
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <InputGroup label="Household ID" placeholder="HH-XXXX" /> {/* Removed Required */}
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white dark:bg-[#1e293b] p-6 rounded-xl border border-neutral-200 dark:border-neutral-700 shadow-sm space-y-5">
                                ...
                                <div className="space-y-4">
                                    ...
                                    {/* Conditional School & Level Fields */}
                                    {isStudying && (
                                    ...
                                    {/* Educational History (Always visible but optional) */}
                                ...
                                <div className="pt-4 border-t border-neutral-100 dark:border-neutral-700 space-y-3">
                                    <div className="flex items-center gap-2 text-rose-500">
                                        ...
                                        <span className="text-xs font-bold uppercase tracking-wider">Family Planning</span>
                                    </div>
                            ...
                            <div className="bg-white dark:bg-[#1e293b] p-6 rounded-xl border border-neutral-200 dark:border-neutral-700 shadow-sm space-y-5">
                                ...
                                <div className="space-y-3 bg-neutral-50 dark:bg-black/20 p-4 rounded-lg border border-neutral-100 dark:border-neutral-700">
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-medium text-neutral-700 dark:text-neutral-300">Registered Voter?</span>
                                        <div className="flex gap-3"><RadioOption name="voter" label="Yes" /><RadioOption name="voter" label="No" defaultChecked /></div>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-medium text-neutral-700 dark:text-neutral-300">Indigenous People (IP)?</span>
                                        <div className="flex gap-3"><RadioOption name="ip" label="Yes" /><RadioOption name="ip" label="No" defaultChecked /></div>
                                    </div>
                                    {/* Conditional Deceased Toggle */}
                                    <div className="flex justify-between items-center">
                                    ...
                                    <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300 bg-red-50 dark:bg-red-900/10 p-3 rounded-lg border border-red-100 dark:border-red-900/30">
                                        ...
                                        <div className="grid grid-cols-3 gap-3">
                                            ...
                                            <input
                                                ...
                                                className="col-span-2 w-full text-xs p-2.5 rounded-lg border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 focus:ring-2 focus:ring-red-500/20 outline-none transition-all"
                                            />
                                        </div>
                                    </div>
                                )}
...
function SectionLabel({ icon, label }: { icon: React.ReactNode, label: string }) {
    return (
        <div className="flex items-center gap-2 pb-2 border-b-2 border-indigo-900/10 dark:border-indigo-500/20">
            <div className="p-1.5 bg-indigo-50 dark:bg-indigo-900/30 rounded text-indigo-700 dark:text-indigo-400">
                {icon}
            </div>
            <h3 className="text-sm font-bold uppercase text-neutral-700 dark:text-neutral-200 tracking-wider">
                {label}
            </h3>
        </div>
    );
    ...
    return (
        <div className={`space-y-1.5 ${className}`}>
            ...
            <div className="relative">
                <select
                    className="w-full text-xs p-2.5 rounded-lg border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none appearance-none transition-all cursor-pointer hover:border-indigo-300"
                    {...props}
                >
