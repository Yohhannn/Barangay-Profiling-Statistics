        ...
        if (isOpen && historyData) {
            setData({
                ...
                histories: [{
                    ...
                    involvement_type: (historyData as any).involvementType || '',
                    case_classification: (historyData as any).caseClassification || ''
                }]
            });
        }
    }, [isOpen, historyData]);
        ...
        put(`/citizen-records/citizen-history/${historyData.id}`, {
            ...
            onSuccess: () => {
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    text: 'Citizen history record(s) updated and logged successfully.',
                });
                handleClose();
            },
            onError: (err) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-[#F8F9FC] dark:bg-[#0f172a] w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-white/20">
                ...
                <div className="relative bg-purple-700 text-white p-5 flex justify-between items-center overflow-hidden shrink-0">
                    ...
                    <div className="z-10">
                        <h2 className="text-lg font-bold tracking-wide flex items-center gap-2">
                            ...
                            Edit Citizen History / Batch Link
                        </h2>
                        <p className="text-[10px] text-purple-200 tracking-widest font-semibold mt-1">
                            Update this record, or link additional citizens and events
                        </p>
                    </div>

                    <button onClick={handleClose} className="z-10 p-2 bg-white/10 hover:bg-white/20 rounded-full transition-all text-white" type="button">
                        <X className="size-5" />
                    </button>
                </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center border-b-2 border-neutral-100 dark:border-neutral-800 pb-2">
                            ...
                            <button
                                ...
                                className="flex items-center gap-1 text-xs font-bold text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 px-3 py-1.5 rounded-lg hover:bg-purple-100 dark:hover:bg-purple-900/40 transition-colors"
                            >
                                <Plus className="size-3.5" /> Link Additional Events
                            </button>
                        </div>
                        ...
                        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700/50 p-4 rounded-xl flex gap-3 text-sm text-yellow-800 dark:text-yellow-200">
                            ...
                            <div>
                                ...
                                <p className="text-xs opacity-80 mt-1">
                                    The first citizen and first history block above will directly update the original record (ID: {historyData.id}). 
                                    Adding more items below them acts as <strong className="font-mono bg-yellow-100 dark:bg-yellow-900/50 px-1 rounded">Batch Editing</strong>. This will create {data.citizens.length * data.histories.length - 1} <b>new</b> independent records in the database simultaneously alongside updating the original.
                                </p>
                            </div>
                        </div>
                    )}
                ...
                <div className="p-5 bg-white dark:bg-[#0f172a] border-t border-neutral-200 dark:border-neutral-800 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] shrink-0">
                    <span className="text-[10px] text-neutral-400 italic">Review updates before saving</span>

                    <button
                        onClick={handleSubmit}
                        disabled={processing}
                        className="flex items-center gap-2 px-8 py-2.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-lg shadow-purple-600/20 active:scale-95"
                    >
...
function CitizenBlock({ index, data, onChange, onRemove, canRemove, errors, isPrimary }: any) {
    const [searchQuery, setSearchQuery] = useState('');
    const [searchResults, setSearchResults] = useState<any[]>([]);
    const [isSearching, setIsSearching] = useState(false);
    const [isLocked, setIsLocked] = useState(data.citizen_id !== null);
    const [hasRecord, setHasRecord] = useState(data.citizen_id !== null);

    // Keep locked state matched with prop data when initialized for editing
    useEffect(() => {
        setIsLocked(data.citizen_id !== null);
        setHasRecord(data.citizen_id !== null);
    }, [data.citizen_id]);
    ...
    return (
        <div className={`bg-white dark:bg-[#1e293b] p-5 rounded-xl border shadow-sm relative ${isPrimary ? 'border-purple-300 dark:border-purple-700 bg-purple-50/30' : 'border-neutral-200 dark:border-neutral-700'}`}>
            {isPrimary && (
                <div className="absolute -top-3 left-4 bg-purple-600 text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-sm">
                    Target Primary Record
                </div>
            )}
            ...
            {isLocked && (
                <div className="flex items-center justify-between p-3 bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-900/50 rounded-lg animate-in fade-in zoom-in duration-300 mb-4 max-w-sm mt-3">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-purple-100 dark:bg-purple-900/40 rounded-full text-purple-600 dark:text-purple-300">
                            <UserCheck className="size-4" />
                        </div>
                        <div>
                            <p className="text-[10px] font-bold text-purple-500 dark:text-purple-400 tracking-wide">Citizen Linked</p>
...
function HistoryBlock({ index, data, onChange, onRemove, canRemove, errors, isPrimary }: any) {
    return (
        <div className={`bg-white dark:bg-[#1e293b] p-5 rounded-xl border shadow-sm relative space-y-4 mt-2 ${isPrimary ? 'border-purple-300 dark:border-purple-700 bg-purple-50/30' : 'border-neutral-200 dark:border-neutral-700'}`}>
             {isPrimary && (
                <div className="absolute -top-3 left-4 bg-purple-600 text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-sm">
                    Target Primary Record
                </div>
