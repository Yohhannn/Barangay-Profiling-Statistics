...
import { MedicalRecord } from '../medical-history';

interface MedicalHistoryEditProps {
    isOpen: boolean;
    onClose: () => void;
    historyData?: MedicalRecord | null;
}

export default function MedicalHistoryEdit({ isOpen, onClose, historyData }: MedicalHistoryEditProps) {
    const { data, setData, put, processing, errors, reset, clearErrors } = useForm({
        citizens: [{ citizen_id: null as number | null, first_name: '', middle_name: '', last_name: '' }],
        histories: [{ type: '', description: '', date_diagnosed: '' }]
    });
        ...
        if (isOpen && historyData) {
            setData({
                ...
                histories: [{
                    ...
                    // The date should just be YYYY-MM-DD
                    date_diagnosed: historyData.dateDiagnosed && historyData.dateDiagnosed !== 'N/A' ? historyData.dateDiagnosed : ''
                }]
            });
        }
    }, [isOpen, historyData]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        put(`/citizen-records/medical-history/${historyData.id}`, {
            preserveScroll: true,
            onSuccess: () => {
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    text: 'Medical history record(s) updated and logged successfully.',
                });
                handleClose();
            },
            onError: (err) => {
            ...
            <div className="bg-[#F8F9FC] dark:bg-[#0f172a] w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-white/20">
                ...
                <div className="relative bg-rose-700 text-white p-5 flex justify-between items-center overflow-hidden shrink-0">
                    ...
                    <div className="z-10">
                        <h2 className="text-lg font-bold tracking-wide flex items-center gap-2">
                            ...
                            Edit Medical History / Batch Link
                        </h2>
                        ...
                        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700/50 p-4 rounded-xl flex gap-3 text-sm text-yellow-800 dark:text-yellow-200">
                            ...
                            <div>
                                ...
                                <p className="text-xs opacity-80 mt-1">
                                    The first citizen and first medical block above will directly update the original record (ID: {historyData.id}). 
                ...
                <div className="p-5 bg-white dark:bg-[#0f172a] border-t border-neutral-200 dark:border-neutral-800 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] shrink-0">
                    ...
                    <button
                        ...
                        disabled={processing}
                        className="flex items-center gap-2 px-8 py-2.5 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-lg shadow-rose-600/20 active:scale-95"
                    >
...
function CitizenBlock({ index, data, onChange, onRemove, canRemove, errors, isPrimary }: any) {
    ...
    return (
        <div className={`bg-white dark:bg-[#1e293b] p-5 rounded-xl border shadow-sm relative ${isPrimary ? 'border-rose-300 dark:border-rose-700 bg-rose-50/30' : 'border-neutral-200 dark:border-neutral-700'}`}>
            {isPrimary && (
                <div className="absolute -top-3 left-4 bg-rose-600 text-white text-[10px] uppercase font-bold px-3 py-1 rounded-full shadow-sm">
...
function HistoryBlock({ index, data, onChange, onRemove, canRemove, errors, isPrimary }: any) {
    return (
        <div className={`bg-white dark:bg-[#1e293b] p-5 rounded-xl border shadow-sm relative space-y-4 mt-2 ${isPrimary ? 'border-rose-300 dark:border-rose-700 bg-rose-50/30' : 'border-neutral-200 dark:border-neutral-700'}`}>
             {isPrimary && (
                <div className="absolute -top-3 left-4 bg-rose-600 text-white text-[10px] uppercase font-bold px-3 py-1 rounded-full shadow-sm">
                    Target Primary Record
                </div>
