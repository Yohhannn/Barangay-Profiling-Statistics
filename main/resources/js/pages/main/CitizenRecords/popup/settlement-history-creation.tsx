...
export default function SettlementHistoryCreation({ isOpen, onClose }: SettlementHistoryCreationProps) {
    const { data, setData, post, processing, errors, reset, clearErrors } = useForm({
        complainants: [
            { id: Math.random().toString(), has_record: false, citizen_id: '', first_name: '', middle_name: '', last_name: '' }
        ],
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (data.linked_history_ids.length === 0) {
            Swal.fire({
                ...
                text: 'You must link at least one history record (Respondent) to this settlement.',
            });
            return;
        }
    ...
    useEffect(() => {
        if (searchQuery.trim().length > 1) {
            ...
            const debounceTimer = setTimeout(() => {
                fetch(`/api/citizen-history/search?q=${encodeURIComponent(searchQuery)}`)
                    .then(res => res.json())
                    .then(data => {
