// js/components/ui/dot-shader-background.tsx
'use client'
...
function Scene() {
  ...
  const getThemeColors = () => {
    switch (theme) {
      ...
      default:
        return { dotColor: '#7fa4f5', bgColor: '#ffffff', dotOpacity: 0.35 }
    }
  }
  ...
  const [trail, onMove] = useTrailTexture({
    ...
    maxAge: 400,
    interpolate: 1,
    ease: (x) => x < 0.5 ? (1 - Math.sqrt(1 - Math.pow(2 * x, 2))) / 2 : (Math.sqrt(1 - Math.pow(-2 * x + 2, 2)) + 1) / 2
  })
